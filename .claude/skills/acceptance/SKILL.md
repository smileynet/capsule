...
**"Is this done, and how do we know?"** Every acceptance report answers this question with evidence. A claim without proof is fiction.
...
4. For epics: confirm all child features have accepted reports, then verify E2E journeys
5. Fill out the template — every criterion needs an evidence column entry
...
- **Over-documentation.** Deliverables tables (file paths are in git), task checklists (beads tracks this), BDD review summaries (captured in bead comments) are internal bookkeeping noise
...
- Review agent verdicts (captured in bead comments, not repeated here)
- Test pass/fail counts (the evidence column links to specific tests)
